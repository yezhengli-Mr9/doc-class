...
7. If you confront HTML page ```502 Bad Gateway``` or ```Method Not Allowed```, it is very likely that you go back to enter [^tn1] ended with ```.compute.amazonaws.com``` (rather than ended with subpage notation ```.../train```, ```.../upload```, ```.../OriginalTraining```, ```.../download1```) and do your test of the system from the scratch.
