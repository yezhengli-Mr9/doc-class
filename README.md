...
1. **Are we able to run your testcases against your webservice? Can we run them against our webservice?** Yes. My test cases are in [doc_class/uploads/](doc_class/uploads/) where for example, row160.csv includes leading 160 lines of original csv. Upload specification appears in the html webservice, that is, the larger the size, the longer the time to upload (although a 273MB file is uploadable).
